...
1. Run `php scripts/populate_featured.php` or create via `$repository->create([...])` with entity_type, entity_id, headline, subheadline, weight, starts_at, ends_at, status
...
- **Persistence API**: `EntityTypeManager::getRepository($type)` returning `Waaseyaa\Entity\Repository\EntityRepositoryInterface` — `getStorage()` throws since alpha.251 (C-22); `find((string)$id)` is string-typed, `findMany()` returns a caller-ordered list (not id-keyed).
...
- **`EntityRepository::delete()` takes a single entity** (`deleteMany()` for arrays) — inverted from the deleted SqlEntityStorage's array contract.
