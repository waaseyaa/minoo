...
Last framework sync: Waaseyaa **alpha.250** (2026-07-02, from alpha.249, PR #918). Suite (530 unit + 154 integration pass), phpstan (level 5), and php-cs-fixer all green. **One breaking change required app changes:** the auth provider now REJECTS an empty/'change-me' reset/verify token HMAC secret outside local/dev/testing environments (`AuthServiceProvider:39` reads `auth.token_secret` ?? `app_secret` — NOT Minoo's pre-existing top-level `jwt_secret` key) and unresolved APP_ENV defaults to 'production', so CI (no .env) 500'd on every kernel-booting integration test. Fixed by (i) mapping `auth.token_secret` from `WAASEYAA_AUTH_TOKEN_SECRET` with fallback to `WAASEYAA_JWT_SECRET` in `config/waaseyaa.php`, and (ii) declaring `APP_ENV=testing` in `phpunit.xml.dist`. **Deploy precondition: verify `WAASEYAA_JWT_SECRET` is non-empty in the Pi's `compose/minoo/secrets/minoo.env` before bumping MINOO_REF, or production boot will throw.** alpha.250 is a security-focused release: SQL identifier quoting in database-legacy (#1817), mail CRLF header-injection rejection (#1824), http-client credential-drop over cross-origin redirects (#1819), cache unserialize integrity guard (#1809), mercure publish-JWT topic scoping (#1821), request body-size cap enforcement vs chunked bypass (#1823), compression Vary-header dedupe (#1822), i18n locale validation before require (#1820). The lagging `engagement`/`graphql`/`testing` packages jumped alpha.157 → alpha.250; `waaseyaa/anokii` moved to alpha.11; `waaseyaa/bimaaji` exact pin moved to `0.1.0-alpha.250`. Prior sync: Waaseyaa **alpha.248** (2026-06-23, from alpha.215, issue #846). Suite (473 pass, 2 skip), phpstan (level 5), and `schema:check` all green; no schema drift. `waaseyaa/bimaaji` is exact-pinned at `0.1.0-alpha.248` to keep its CLI/MCP surface version-matched; all other constraints are `^0.1.0-alpha.248`. Keep-list breaking changes across alpha.216-248 that required Minoo code changes: (i) **CLI command model returned to Symfony Console.** `HasNativeCommandsInterface` → `ProvidesConsoleCommandsInterface` (method `nativeCommands()` → `consoleCommands()`); `Waaseyaa\CLI\{CommandDefinition,ArgumentDefinition,ArgumentMode}` removed in favour of `Waaseyaa\CLI\Command\{HandlerCommand,HandlerArgument,HandlerArgumentMode}`; `Waaseyaa\CLI\CliIO` → `Waaseyaa\CLI\Command\SymfonyCommandIO`. Updated `AppCommandServiceProvider` + `App\Console\MailTestHandler`. (ii) **Framework search is now access-checked.** The `SearchProviderInterface` factory transitively resolves `Waaseyaa\Access\EntityAccessHandler`, which the kernel composes in `discoverAccessPolicies()` — *after* `bootProviders()`. `AppBootServiceProvider::boot()` registers the search Twig extension (Twig locks extensions at first render, so it must be added at boot), and eagerly resolving the provider there threw `No binding registered for Waaseyaa\Access\EntityAccessHandler`. Fixed with `App\Search\LazySearchProvider`, which defers provider resolution (and its access-handler dependency) to first query — mirroring the framework's own lazy access-handler accessors. (iii) **bimaaji MCP went framework-native** (no Node runtime; tools exposed via the framework `/mcp` endpoint with `#[AsAgentTool]` adapters). The standalone `vendor/waaseyaa/bimaaji/mcp/server.js` is gone — removed the dead `bimaaji` stdio entry from `.claude/settings.json`, and `bimaaji-mcp-install` / `bin/install-mcp-npm.php` now only install Minoo's own `mcp/` deps. Re-register bimaaji for a client via `bin/waaseyaa bimaaji:install --client=claude` if direct tool access is wanted. (iv) **New framework packages** pulled in additively (not yet consumed by Minoo): `waaseyaa/wayfinding` (flagship beacon/trail subsystem) + `waaseyaa/seo`. Prior sync: Waaseyaa **alpha.215** (2026-06-15, Phase 2 relaunch bump from alpha.209) — keep-list-relevant breaking changes across alpha.204-215: (a) **save-time entity validation enforced framework-wide** — `EntityRepository::save()`/`saveMany()` reject entities that violate their field definitions with `EntityValidationException` BEFORE any write (loosely-typed data like a numeric string in an `integer` field can be rejected unless a cast absorbs it; opt out per-call with `save($e, validate: false)` or globally with `WAASEYAA_ENTITY_VALIDATION=0`). Matters for Phase 3 data import + new field defs. (b) **denied single reads return 404, not 403** on the generic `GET /api/{type}/{id}` (closes an existence oracle); Minoo's custom controllers do not rely on that endpoint's 403. (c) alpha.214 wired the stock agent entity-tools access gate fail-closed and fixed a latent OIDC `/userinfo` 500 — not used by Minoo surfaces. (d) alpha.212 `core` metapackage now requires `waaseyaa/scheduler`. Earlier sync notes follow (alpha.182, 2026-05-19); highlights since alpha.175:
