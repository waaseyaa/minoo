...
> **⚠️ SUPERSEDED (same day) by [FEEDS-SERVICE-SPEC.md](feeds-service.md)** — the fetcher moves out of minoo into a standalone shared worker serving minoo + rhtcircle.ca + oiatc.ca (events **and news**). The pipeline design below (§2–§6) carries forward; the placement (§1 minoo-internal, `config/event_sources.php`, minoo-side fetch command) does not.
