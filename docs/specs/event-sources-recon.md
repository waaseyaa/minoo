...
| 5 | Serpent River | ★ | serpentriverfn.com | Yes (200) | WordPress/Avada/Divi | `/events/` (HTML), Tribe Events plugin declared (`wp-json/tribe/events/v1/`) | Declared (`<link rel="alternate" type="text/calendar">` at `/events/?ical=1`) but **fetch returns HTTP 200, `Content-Length: 0`, `X-Robots-Tag: noindex`** — looks like a bot-mitigation empty response to this UA, not usable as-is | Yes, `/feed/`, works; newest sampled post 2026-01-16 (~5.5 months stale) | `/news/` redirects → `/newsletter/` | 2026-01-16 | review | ICS is declared but unconfirmed — **blocked_pending_outreach** (see bot-wall policy in the rollup below). |
...
**Declared ICS, not currently usable:**
- Serpent River, Atikameksheng — `?ical=1` returns an empty `200`/`Content-Length: 0` to this UA (looks like bot mitigation on the dynamic endpoint). **Status: `blocked_pending_outreach`.** Bot-wall policy (Russell, 2026-07-02): **we do not evade bot mitigation on community partner sites, ever** — no UA rotation, no browser impersonation. OIATC will ask these nations directly to allowlist `RHTFeedsBot`; the endpoints stay out of the registry until that outreach lands.
- Wiikwemkoong — `?ical=1` returned `502` at fetch time (transient server error, not bot mitigation) — plain re-check later is fine.
...
- Declared-but-broken ICS endpoints are **excluded** from the draft registry rather than included and pre-broken. Serpent River and Atikameksheng are `blocked_pending_outreach` — per the bot-wall policy above, the path forward is OIATC asking those nations to allowlist `RHTFeedsBot`, never technical evasion. Wiikwemkoong's 502 is an ordinary transient failure; re-check it when the real fetcher exists.
- **Auto-candidacy rule (final):** `auto_candidate` requires **nation-owned AND own-domain AND confirmed-working** — all three. Thessalon's ICS is nation-curated but Google-hosted, so it caps at `review`. Anishinabek Nation (umbrella org, not one of the 21) is **included at review-tier and can never be auto** — not nation-owned.
- **Sagamok canonical domain is `sagamokanishnawbek.com`** — `sagamok.ca` (used in minoo's `MamaweswenNations.php`) is a 301 alias; the registry points at the canonical domain.
