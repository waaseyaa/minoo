...
| `Site/` | `App\Http\Controller\Site` | `NewsletterApiRouteProvider` (`StaticPageController`, static marketing/info pages) |
...
## Admin SPA (not a `Site/` controller)

The main admin shell is served by route `admin_spa` in `AdminRouteProvider` (closure + `AdminSurfaceServiceProvider::resolveAdminIndex()` / vendor `admin-surface` dist). There is no `App\Http\Controller\Site\AdminController` in the routing graph.
