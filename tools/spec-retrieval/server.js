...
server.tool(
  "minoo_get_spec",
  "Retrieve the full content of a Minoo subsystem spec by name. Use this when you need detailed interface signatures, data flows, file maps, or edge cases for a specific subsystem. Available specs: entity-model, ingestion-pipeline, search, frontend-ssr.",
  { name: z.string().describe("Spec name without .md extension, e.g. 'entity-model', 'ingestion-pipeline', 'search', 'frontend-ssr'") },
  async ({ name }) => {
    const specs = await loadSpecs();
